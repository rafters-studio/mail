...
describe("bulkActionSchema", () => {
  ...
  it("rejects moveToFolder without folderId", () => {
    expect(() => bulkActionSchema.parse({ ids: ["1"], action: "moveToFolder" })).toThrow();
  });
  ...
  it("rejects setStatus without status", () => {
    expect(() => bulkActionSchema.parse({ ids: ["1"], action: "setStatus" })).toThrow();
  });

  it("rejects setPriority without priority", () => {
    expect(() => bulkActionSchema.parse({ ids: ["1"], action: "setPriority" })).toThrow();
  });
